...
public abstract class EarthquakeMarker extends CommonMarker implements Comparable<EarthquakeMarker>
{
	...
	public EarthquakeMarker (PointFeature feature) 
	{
	 ...
	 public int compareTo(EarthquakeMarker marker) {
		 float currMag = Float.parseFloat(this.getProperty("magnitude").toString());
		 float compMag = Float.parseFloat(marker.getProperty("magnitude").toString());
		 if (currMag < compMag) {
			 return -1;
		 } else if (currMag > compMag) {
			 return 1;
		 }
		 return 0;
	 }
	...
	public void showTitle(PGraphics pg, float x, float y)
	{
		...
		pg.noFill();
		pg.ellipse(x, y, (float) (threatCircle()/20), (float) (threatCircle()/20));
