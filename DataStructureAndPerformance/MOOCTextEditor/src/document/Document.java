...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		Pattern sylSplitter = Pattern.compile("[^AIUEOYaiueoy]*[AIUEOYaiueoy]+[^AIUEOYaiueoy]*");
		Matcher m = sylSplitter.matcher(word);
		...
		String lastSyl = syllables.get(syllables.size()-1);
		Matcher e = Pattern.compile("[AIUEOYaiueoy]").matcher(lastSyl);
		int countE = 0;
		while(e.find()) {
			countE += 1;
		}
		
		if (lastSyl.charAt(lastSyl.length()-1) == 'e' & countE == 1){
